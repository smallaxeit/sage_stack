...
export default function AdminPanel({ open, onClose, buildProgress }) {
  ...
  const embPct = stats?.chunks > 0 ? Math.min(100, Math.round((stats.embeddings / stats.chunks) * 100)) : 0;
          ...
          {tab === 'status' && (
            <>
              <Section title="System">
                ...
                {stats?.traditionsList?.length > 0 && (
                  <div className="admin-traditions-list">
                    {stats.traditionsList.map(t => (
                      <span key={t.name || t} className="admin-tradition-chip">{t.name || t}</span>
                    ))}
                  </div>
                )}
                {stats?.builtAt && (
